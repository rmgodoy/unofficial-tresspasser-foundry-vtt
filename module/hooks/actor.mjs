...
export function registerActorHooks() {
  ...
  Hooks.on("createActor", async (actor, options, userId) => {
    ...
    if (actor.passiveStates?.encumbered) {
      await TrespasserEffectsHelper.syncActorEncumberedItem(actor);
    }
  });
  ...
  Hooks.on("updateActor", async (actor, updateData, options, userId) => {
    ...
    if (foundry.utils.hasProperty(updateData, "system.armor") || foundry.utils.hasProperty(updateData, "system.equipment")) {
      await TrespasserEffectsHelper.syncActorEncumberedItem(actor);
    }
  ...
  Hooks.on("updateActor", async (actor, updates, options, userId) => {
    const result = foundry.utils.getProperty(updates, `flags.${SYSTEM_ID}.initiativeRollResult`);
    if (result && game.user.isGM) {
      ...
      await actor.unsetFlag(SYSTEM_ID, "initiativeRollResult");
    }
  });
}
